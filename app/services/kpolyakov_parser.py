...
class KpolyakovParser:
    ...
    def _extract_from_script(self, script_tag: Tag) -> str:
        ...
        if match:
            ...
            def resolve_img(m):
                ...
                return f'<img src="{url}" class="img-fluid question-image" style="display:inline;vertical-align:middle;max-height:60px;margin:2px">'
    ...
    def _extract_files(self, topic_td: Tag) -> List[str]:
        file_urls = []
        for script in topic_td.find_all("script"):
            ...
            for match in re.finditer(r'<a\s+href="([^"]+)"', text):
                ...
                if href.startswith("http"):
                    ...
                    file_urls.append(f"https://kpolyakov.spb.ru{href}")
                else:
                    file_urls.append(f"{FILE_BASE_URL}/{href}")
        return file_urls
    ...
    def parse_page(self, url: str = None, html: str = None, task_number: int = 0, db_session=None) -> Dict:
        ...
        try:
            ...
            while i < len(rows):
                ...
                file_urls = self._extract_files(topic_td)
